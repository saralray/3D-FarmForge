...
**Programmatic data API (`/api/v1`):** A versioned, API-key-gated API over the print-farm's data, served by the `web` server (`handleDataApi` in `server/app.js`) and entirely separate from the cookieless frontend `/api/*` endpoints (which stay unauthenticated). Keys are the **same `slicer_api_keys`** minted in Settings → Slicer Upload (sha256-hashed); a caller passes one via the `X-Api-Key` header or `Authorization: Bearer <key>`. Any valid key grants **full read/write**; usage stamps `last_used_at` and every mutation is written to the audit log with `source = 'api'`. Because the key is the guard, printer connection details are **not** redacted here (unlike the public-viewer `listPrinters` path). Resources (`GET /api/v1` lists them): `printers` (list/get/upsert/delete, plus `POST /printers/:id/command` for Bambu), `queue` (list stored jobs/upsert/`:id/printed`/delete/`reset` — GET does **not** trigger a Google Sheet sync), `analytics` (daily rollups + `reset`), `notifications` (Discord webhook CRUD), `slicer-keys` (list/mint/revoke; plaintext returned once), `audit-logs` (read/append), and `settings/<key>` (app_settings GET/PUT). A missing/invalid key returns 401.
